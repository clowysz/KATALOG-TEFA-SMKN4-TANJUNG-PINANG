...
<div class="login-container" style="display: flex; justify-content: center; align-items: center; min-height: 100vh;">
    <div class="tefa-card login-box" style="background: white; width: 100%; max-width: 400px; padding: 40px; border-radius: 16px; border: 1px solid #E2E8F0; box-shadow: 0 4px 20px rgba(0,0,0,0.03);">
        ...
        <form
            action="{{ route('admin.login.proses') }}"
            method="POST"
            id="formLoginProduser"
        >
            @csrf
            ...
            <input
                ...
                required
                style="width: 100%; padding: 12px 16px; border-radius: 8px; border: 1px solid #E2E8F0; margin-bottom: 16px; box-sizing: border-box;"
            >
            ...
            @error('email')
                <div style="color: #DC2626; font-size: 13px; margin-top: 12px; background: #FEE2E2; padding: 10px; border-radius: 8px; text-align: center;">
                    {{ $message }}
                </div>
            @enderror
            ...
            <button
                ...
                id="btnLogin"
                class="btn-primary"
                style="background: #1E3A8A; color: white; border: none; font-weight: 600; margin-top: 24px; width: 100%; border-radius: 8px; padding: 14px; cursor: pointer; transition: 0.3s;"
            >
                Masuk
            </button>
        </form>
...
<script>
    ...
    // Spinner saat form dikirim
    document.getElementById('formLoginProduser').addEventListener('submit', function() {
