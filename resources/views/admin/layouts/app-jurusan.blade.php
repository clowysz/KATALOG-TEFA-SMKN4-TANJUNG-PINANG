...
<body>
    ...
    <nav class="drawer" id="navDrawer">
        ...
        <div class="drawer-footer">SMKN 4 Tanjungpinang © 2026</div>
    </nav>
        ...
        <div class="modal-logout-box">
            ...
            <div class="modal-logout-actions">
                ...
                <form action="{{ route('admin.logout') }}" method="POST" style="flex:1;margin:0;">
                    @csrf
                    <button type="submit" class="btn-modal-logout" style="width:100%;cursor:pointer;">Logout</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <style>
        .modal-logout-overlay{position:fixed;inset:0;background:rgba(15,23,42,.45);backdrop-filter:blur(2px);display:flex;align-items:center;justify-content:center;z-index:99999}
        .modal-logout-box{background:#fff;padding:32px;border-radius:20px;width:90%;max-width:440px;box-shadow:0 20px 25px -5px rgba(0,0,0,.1);text-align:left;animation:modalFadeIn .2s ease-out}
        @keyframes modalFadeIn{from{opacity:0;transform:scale(.95)}to{opacity:1;transform:scale(1)}}
        .modal-logout-title{font-size:20px;font-weight:700;color:#20456E;margin:0 0 8px;font-family:sans-serif}
        .modal-logout-desc{font-size:14px;color:#64748B;margin:0 0 24px;font-family:sans-serif}
        .modal-logout-actions{display:flex;align-items:center;gap:12px}
        .btn-modal-cancel{background:#fff;color:#334155;border:1px solid #CBD5E1;padding:12px 24px;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;transition:.2s}
        .btn-modal-cancel:hover{background:#F8FAFC}
        .btn-modal-logout{display:block;background:#DC2626;color:#fff;border:none;padding:12px 24px;border-radius:10px;font-size:15px;font-weight:700;text-align:center;transition:.2s}
